...
enum HeatTone { neutral, good, bad, rival, terrain }

class HeatLine {
  HeatLine(this.text, this.tone, this.turn);
  final String text;
  final HeatTone tone;
  final int turn;
}

enum HeatPhase { racing, finished }
...
class HeatEngine implements HeatApi {
  HeatEngine({
    required this.track,
    required Contender player,
    required List<Contender> rivals,
    required this.rng,
    this.laneCount = 3,
  }) : _player = player,
  ...
  final Contender _player;
  final List<Contender> entrants;

  int turn = 1;
  int effort = 0;
  HeatPhase phase = HeatPhase.racing;
  ...
  Contender get player => _player;

  List<Contender> get rivals =>
  ...
  void _buildDeck() {
    deck
      ..clear()
      ..addAll(_player.phenotype.maneuverIds);
    rng.shuffle(deck);
  }
  ...
  void beginTurn() {
    if (phase == HeatPhase.finished) return;
    ...
    for (final e in rivals) {
      ...
      e.intent = Archetype.byId(e.archetypeId ?? 'pacer').plan(
        RivalSight(self: e, field: entrants, track: track, turn: turn, rng: rng),
      );
    }
  }
  ...
  void endTurn() {
    ...
    if (phase == HeatPhase.racing) beginTurn();
  }
  ...
  bool canPlay(int handIndex) {
    if (phase != HeatPhase.racing) return false;
    if (handIndex < 0 || handIndex >= hand.length) return false;
    return Maneuvers.byId(hand[handIndex]).effort <= effort;
  }
  ...
  bool play(int handIndex, {int? lane}) {
    ...
    final command = Maneuvers.byId(id);
    ...
    _pendingLane = lane;
    _log('${_player.name}: ${command.name}', HeatTone.neutral);
    command.effect(this);
    _pendingLane = null;
  ...
  @override
  Contender get actor => _player;

  @override
  List<Contender> get field => entrants;
  ...
  @override
  Contender? get rivalAhead => _drafteeFor(_player);
  ...
  void move({int bonus = 0, int staminaCost = 0, int extra = 0}) {
    ...
    if (e.blown && cost > 0) {
      _log('${e.name} is out of wind.', HeatTone.bad);
    }
  }
  ...
  void changeLane([int? lane]) {
    ...
    if (e.status(Status.clipped) > 0) {
      _log('${e.name} is clipped and cannot switch lanes.', HeatTone.bad);
      return;
    }
    ...
    if (_laneBlocked(e, target)) {
      _log('Lane ${target + 1} is occupied.', HeatTone.bad);
      return;
    }
    e.lane = target;
    _log('${e.name} moves to lane ${target + 1}.', HeatTone.neutral);
  }
  ...
  @override
  void log(String message) => _log(message, HeatTone.neutral);
  ...
  List<Contender> get standings {
    final sorted = List<Contender>.of(entrants);
    sorted.sort((a, b) {
  ...
  Contender? _drafteeFor(Contender e) {
    Contender? best;
  ...
  bool _laneBlocked(Contender e, int lane) {
    for (final other in entrants) {
  ...
  int _terrainStaminaSurcharge(Contender e) {
  ...
  void _advance(Contender e, double ground) {
    if (ground <= 0) {
      e.lastGain = 0;
      return;
    }
    ...
    for (final other in entrants) {
      ...
      if (target > other.distance - 1) {
        ...
        _log('${other.name} holds the line against ${e.name}.', HeatTone.rival);
      }
    }
  ...
  void _resolveRivals() {
    final order = List<Contender>.of(rivals)
  ...
  void _rivalMove(Contender e, int bonus, int staminaCost) {
  ...
  void _rivalChangeLane(Contender e, int? lane) {
  ...
  void _rivalClip(Contender e) {
    ...
    _log('${e.name} clips ${target.name}.', HeatTone.bad);
  }
      ...
      switch (terrain) {
        case Terrain.corner:
          ...
          if (burn > 0) {
            ...
            if (e.isPlayer) {
              _log('The corner burns $burn stamina.', HeatTone.terrain);
            }
          }
        ...
        case Terrain.puddle:
          if (e.grip < 2 && !_consumeFlap(e)) {
            if (e.momentum > 0 && e.isPlayer) {
              _log('The puddle kills your momentum.', HeatTone.terrain);
            }
            e.momentum = 0;
          }
        case Terrain.hay:
          if (!_consumeFlap(e) && e.grip < 1) {
            ...
            if (e.isPlayer) {
              _log('Hay bales cost you 2 ground.', HeatTone.terrain);
            }
          }
        case Terrain.gravel:
          if (e.grip < 2 && !_consumeFlap(e)) {
        ...
        case Terrain.crowd:
          if (leader.id == e.id && e.isPlayer) {
            effort += 1;
            _log('The crowd carries you: +1 effort.', HeatTone.good);
    ...
    for (final e in standings) {
      ...
      if (e.distance >= track.totalLength) {
        ...
        _log(
          ...
          e.isPlayer ? HeatTone.good : HeatTone.rival,
        );
      }
    }
  ...
  HeatResult get result => HeatResult(
  ...
  void _log(String text, HeatTone tone) {
    log_.add(HeatLine(text, tone, turn));
