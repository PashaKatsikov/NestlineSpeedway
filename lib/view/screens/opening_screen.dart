...
class _OpeningScreenState extends State<OpeningScreen> {
  ...
  void _finish() {
    context.read<Director>().completeIntro();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Stack(
        fit: StackFit.expand,
        children: [
          Image.asset(Backdrops.trackAutumn, fit: BoxFit.cover),
          DecoratedBox(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [
                  ...
                  Pigment.pitch.withValues(alpha: 0.95),
                ],
              ),
            ),
          ),
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Text(chapter.title, style: Face.title(25)),
                  const SizedBox(height: 9),
                  Text(chapter.body, style: Face.text(12.5, height: 1.5)),
                ],
              ),
            ),
          ),
      ...
      children: [
        for (var i = 0; i < count; i++)
          AnimatedContainer(
            ...
            height: 8,
            decoration: BoxDecoration(
              color: i == active ? Pigment.amber : Pigment.slateHi,
...
class _ManeuversArt extends StatelessWidget {
  const _ManeuversArt();
...
class _HeredityArt extends StatelessWidget {
  const _HeredityArt();
  ...
  Widget build(BuildContext context) {
    ...
    if (racer == null) {
      return const VacantNote('Your first birds are on their way.');
    }
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            HenView(pose: racer.portraitPose, plume: racer.plume, size: 46),
        ...
        return Column(
          ...
          children: [
            GaugeBar(
              value: stamina.round(),
              max: 14,
              tint: stamina < 4 ? Pigment.staminaLow : Pigment.stamina,
            ...
            Row(
              ...
              children: [
                ...
                Icon(
                  Icons.air_rounded,
                  size: 18,
                  color: Pigment.momentum.withValues(alpha: 0.8),
                ),
                const SizedBox(width: 4),
                const HenView(
...
class _Cost extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          Text(value, style: Face.number(17, color: tint)),
          Text(
            label,
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
            style: Face.text(10, color: Pigment.inkSoft),
          ),
        ],
      ),
    );
    ...
    return Column(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const HenView(pose: Atlas.poseReady, plume: 2, size: 52),
            const _Glyph(Icons.add_rounded),
            const HenView(pose: Atlas.poseCalm, plume: 11, size: 52),
            const _Glyph(Icons.arrow_forward_rounded),
            Image.asset(Atlas.egg(3), width: 44, height: 44),
          ],
        ),
        const SizedBox(height: 12),
        Wrap(
          ...
          runSpacing: 7,
          children: [
            for (final tier in ShellTier.all.take(4))
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(Corners.rSm),
                  border: Border.all(
                    color: Pigment.tiers[tier.index].withValues(alpha: 0.5),
                  ),
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Image.asset(Atlas.egg(tier.index), width: 18, height: 18),
                    const SizedBox(width: 5),
                    Text(
                      tier.name,
                      style: Face.text(10, color: Pigment.tiers[tier.index]),
                    ),
                  ],
                ),
              ),
...
class _Glyph extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Icon(icon, size: 17, color: Pigment.inkMute),
    );
  }
}
...
class _RouteArt extends StatelessWidget {
  ...
  static const List<StopKind> _route = [
    StopKind.sprint,
    StopKind.trader,
    StopKind.endurance,
    StopKind.rest,
          ...
          for (var i = 0; i < _route.length; i++) ...[
            ...
            if (i < _route.length - 1)
              Icon(
                ...
                size: 15,
                color: Pigment.inkMute.withValues(alpha: 0.7),
              ),
          ],
        ],
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(Corners.rSm),
        border: Border.all(
          color: open ? tint : Pigment.slate,
          width: open ? 2 : 1,
        ),
        boxShadow: open ? Corners.glow(tint, 0.35) : null,
      ),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(kind.icon, size: 16, color: open ? tint : Pigment.inkMute),
          const SizedBox(width: 7),
          Text(
            kind.label,
            style: Face.text(11, color: open ? Pigment.ink : Pigment.inkSoft),
          ),
        ],
      ),
    );
