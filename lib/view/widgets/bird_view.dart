...
/// roster, the Brooder and the race lanes.
class HenView extends StatelessWidget {
  const HenView({
    super.key,
    required this.pose,
    required this.plume,
  ...
  Widget build(BuildContext context) {
    final bird = Image.asset(
      ...
      color: dim ? Pigment.pitch.withValues(alpha: 0.55) : null,
...
class LivingBird extends StatefulWidget {
  const LivingBird({
    super.key,
    required this.pose,
    required this.plume,
  ...
  @override
  State<LivingBird> createState() => _LivingBirdState();
}

class _LivingBirdState extends State<LivingBird>
...
class RunnerCard extends StatelessWidget {
  ...
  List<(String, Color)> get _tags {
    ...
    return [
      if (racer.races > 0) ('${racer.wins}W / ${racer.races}', Pigment.amber),
      if (pheno.pureTraits.isNotEmpty)
        ('${pheno.pureTraits.length} pure', Pigment.amber),
      if (pheno.synergies.isNotEmpty)
        ('${pheno.synergies.length} synergy', Pigment.schoolRainbow),
      if (racer.injuries.isNotEmpty)
        ('${racer.injuries.length} injury', Pigment.bad),
      if (racer.fatigue > 40) ('${racer.fatigue} fatigue', Pigment.warn),
      if (racer.inbreedingPenalty.label.isNotEmpty)
        (racer.inbreedingPenalty.label, Pigment.bad),
    ];
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Pane(
      onTap: onTap,
      selected: selected,
      accent: racer.careerOver ? Pigment.bad : Pigment.amber,
                      ...
                      children: [
                        Flexible(
                          child: Text(
                            ...
                            style: Face.title(compact ? 14 : 16),
                          ),
                        ),
                        if (racer.rank > 0) ...[
                          const SizedBox(width: 6),
                          _RankDots(rank: racer.rank),
                        ],
                      ],
                    ),
                    if (showStats) ...[
                      ...
                      Text(
                        ...
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: Face.text(compact ? 10.5 : 11.5),
                      ),
                    ],
                    if (badges) ...[
...
class _RankDots extends StatelessWidget {
  const _RankDots({required this.rank});
  final int rank;
...
class _TinyTag extends StatelessWidget {
  const _TinyTag(this.text, this.tint);
  final String text;
  final Color tint;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(text, style: Face.text(9.5, color: tint)),
    );
  }
}
              ...
              children: [
                ...
                if (showNumbers)
                  Text(
                    ...
                    style: Face.number(11, color: tint),
                  ),
              ],
            ),
          ),
        ...
        return Padding(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              shape: BoxShape.circle,
              border: Border.all(
                color: on ? tint : Pigment.slateHi,
                width: on ? 0 : 1.5,
              ),
              boxShadow: on ? Corners.glow(tint, 0.35) : null,
            ),
          ),
        );
      }),
