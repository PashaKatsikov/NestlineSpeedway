...
class AlleleMark extends StatelessWidget {
  const AlleleMark({
    super.key,
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: known
              ...
              : Pigment.slate.withValues(alpha: 0.5),
          borderRadius: BorderRadius.circular(6),
          border: Border.all(
            color: known ? tint.withValues(alpha: 0.8) : Pigment.slateHi,
          ),
        ),
        child: Text(
          known ? allele.code : '?',
          style: Face.number(
            size * 0.52,
            color: known ? tint : Pigment.inkMute,
          ),
        ),
      ),
    );
...
/// The six loci of a bird, with the carried allele masked until the Ledger has
/// identified it.
class HeredityStrip extends StatelessWidget {
  const HeredityStrip({
    super.key,
    required this.racer,
    required this.codex,
  ...
  /// Set by the Bloodline Office upgrade: always show both alleles.
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: Text(
                        ...
                        style: Face.label(dense ? 8 : 8.5),
                      ),
                    ),
                    AlleleMark(allele: g.expressed(l), size: chip),
...
class TraitStrip extends StatelessWidget {
  const TraitStrip({super.key, required this.phenotype});
    ...
    return Column(
      ...
      children: [
        Wrap(
          ...
          runSpacing: 6,
          children: [
            _Tag('${phenotype.staminaMax} wind', Pigment.stamina),
            _Tag('${phenotype.stride} stride', Pigment.ember),
            _Tag('${phenotype.effort} effort', Pigment.effort),
            _Tag('${phenotype.grip} grip', Pigment.momentum),
            _Tag('${phenotype.control} control', Pigment.distance),
            _Tag('${phenotype.recovery} recovery', Pigment.stamina),
            _Tag('${phenotype.hand} hand', Pigment.inkSoft),
          ],
        ),
        if (phenotype.pureTraits.isNotEmpty) ...[
          const SizedBox(height: 9),
          Text('PURE TRAITS', style: Face.label(8.5)),
          ...
          Wrap(
            ...
            children: [
              for (final a in phenotype.pureTraits)
                _Tag('${a.name} ${a.locus.label}', Pigment.amber),
            ],
          ),
        ],
        if (phenotype.synergies.isNotEmpty) ...[
          const SizedBox(height: 9),
          Text('SYNERGIES', style: Face.label(8.5)),
          const SizedBox(height: 4),
              ...
              child: Row(
                children: [
                  Icon(
                    Icons.auto_awesome,
                    size: 12,
                    color: Pigment.schoolRainbow,
                  ),
                  const SizedBox(width: 5),
                  Expanded(
                    child: Text(
                      '${s.name} — ${s.blurb}',
                      style: Face.text(10.5, color: Pigment.schoolRainbow),
                    ),
                  ),
                ],
              ),
...
class _Tag extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(text, style: Face.text(10, color: tint)),
    );
  }
}
...
class PairingOutlook extends StatelessWidget {
  const PairingOutlook({
    super.key,
    required this.sire,
    required this.dam,
    required this.codex,
          ...
          Padding(
            ...
            child: Row(
              children: [
                SizedBox(
                  width: 62,
                  child: Text(l.label.toUpperCase(), style: Face.label(8.5)),
                ),
                Expanded(
                  child: Wrap(
                    spacing: 5,
                ...
                SizedBox(
                  width: 62,
                  child: Text(
                    'pure ${(Brooder.pureChance(sire, dam, l) * 100).round()}%',
                    textAlign: TextAlign.right,
                    style: Face.text(
                      9.5,
                      color: Brooder.pureChance(sire, dam, l) > 0
                          ? Pigment.amber
                          : Pigment.inkMute,
                    ),
                  ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(
        ...
        style: Face.text(10, color: known ? tint : Pigment.inkMute),
      ),
    );
  }
}
