...
class Injury {
  ...
  static Injury roll(Dice rng, int maxSeverity) {
...
class Runner {
  ...
  Build phenotype({StatMods extra = const StatMods()}) {
    ...
    var commands = List<String>.of(base.maneuverIds);
    if (penalty.commandLoss > 0) {
      // Drop the weakest non-core commands first.
      final core = Build.baseManeuvers.toSet();
  ...
  int get portraitPose {
    if (careerOver) return Atlas.poseSpent;
    if (injuries.isNotEmpty) return Atlas.poseHurt;
    if (fatigue > 60) return Atlas.poseCalm;
    if (wins > 0) return Atlas.poseStrut;
    return Atlas.poseReady;
  }
  ...
  static Runner fromJson(Map<String, dynamic> j) => Runner(
    id: j['id'] as String,
    name: j['name'] as String,
    genome: Heredity.fromJson(j['g'] as List<dynamic>),
