...
class ShellTier {
  const ShellTier(this.index, this.name, this.blurb, this.mutation, this.bias);
  ...
  static const List<ShellTier> all = [
    ShellTier(
      0,
      'Plain',
    ...
    ShellTier(1, 'Speckled', 'Steadier hatch. Slightly kinder odds.', 0.02, 0.1),
    ShellTier(
      2,
      'Burnished',
    ...
    ShellTier(4, 'Prism', 'Unstable and generous in equal measure.', 0.05, 0.5),
    ShellTier(
      5,
      'Crystal',
    ...
    ShellTier(6, 'Heirloom', 'The best shell on the circuit.', 0.01, 0.7),
  ];

  static ShellTier at(int i) => all[i.clamp(0, all.length - 1)];
}
...
class Brooder {
  ...
  static String pickName(Dice rng, Iterable<String> taken) {
  ...
  static List<Runner> founders(Dice rng, Bloodline pedigree) {
    final out = <Runner>[];
    final taken = <String>[];
    for (var i = 0; i < 3; i++) {
      final genome = Heredity.random(rng, recessiveBias: 0.15);
      ...
      pedigree.register(KinNode(id: id, name: name, birthOrder: order));
      out.add(
        Runner(
          id: id,
          name: name,
  ...
  static Runner wildStock(
    Dice rng,
    Bloodline pedigree, {
    required int grade,
    Iterable<String> taken = const [],
  }) {
    final genome = Heredity.random(rng, recessiveBias: 0.1 + grade * 0.06);
    final name = pickName(rng, taken);
    final id = newId();
    final order = pedigree.nextBirthOrder;
    pedigree.register(KinNode(id: id, name: name, birthOrder: order));
  ...
  static Runner breed({
    required Runner sire,
    required Runner dam,
    required ShellTier tier,
    required Dice rng,
    required Bloodline pedigree,
    Iterable<String> taken = const [],
  }) {
    final genome = Heredity.breed(
      sire.genome,
      dam.genome,
      rng,
  ...
  static List<Runner> claimClutch(
    Dice rng,
    Bloodline pedigree, {
    Iterable<String> taken = const [],
  }) {
  ...
  static int _plumeFor(Heredity g, Dice rng) {
  ...
  static Map<Locus, Map<int, double>> forecast(Runner sire, Runner dam) {
  ...
  static double pureChance(Runner sire, Runner dam, Locus l) {
    final sireOptions = [sire.genome.first(l), sire.genome.second(l)];
    final damOptions = [dam.genome.first(l), dam.genome.second(l)];
    var hits = 0;
    for (final a in sireOptions) {
