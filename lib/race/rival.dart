...
class RivalView {
  ...
  int get positionIndex {
    final sorted = List<Entrant>.of(field)
...
class Archetype {
  ...
  final String name;
  final String blurb;
  final Intent Function(RivalView view) plan;
  ...
  static final List<Archetype> all = [
    Archetype(
      ...
      strideBonus: 1,
      plan: (v) {
        if (v.staminaFraction < 0.18) return const Intent(IntentKind.steady);
        if (v.raceProgress < 0.62) {
          return Intent(IntentKind.surge, magnitude: v.isLeading ? 2 : 4);
        }
        if (v.staminaFraction > 0.45) {
          return const Intent(IntentKind.surge, magnitude: 3);
        }
        return const Intent(IntentKind.cruise, magnitude: 1);
      },
    ),
    Archetype(
      id: 'closer',
      ...
      plan: (v) {
        if (v.raceProgress > 0.72) {
          ...
          return const Intent(IntentKind.surge, magnitude: 5);
        }
        if (v.aheadSameLane != null) return const Intent(IntentKind.draft);
        final target = v.player?.lane ?? v.self.lane;
        if (target != v.self.lane) {
          return Intent(IntentKind.cut, targetLane: target);
        }
        return const Intent(IntentKind.cruise, magnitude: 1);
      },
    ),
    Archetype(
      ...
      staminaScale: 1.2,
      plan: (v) {
        if (v.staminaFraction < 0.35) return const Intent(IntentKind.steady);
        if (v.terrainNext == Terrain.corner && v.self.momentum > 2) {
          return const Intent(IntentKind.steady);
        }
        return const Intent(IntentKind.cruise, magnitude: 2);
      },
    ),
    Archetype(
      id: 'bruiser',
      ...
      plan: (v) {
        ...
        if (player != null &&
          ...
          return const Intent(IntentKind.clip);
        }
        if (v.isLeading) return const Intent(IntentKind.block);
        if (player != null && player.lane != v.self.lane && v.rng.chance(0.5)) {
          return Intent(IntentKind.cut, targetLane: player.lane);
        }
        return const Intent(IntentKind.cruise, magnitude: 2);
      },
    ),
      ...
      plan: (v) {
        ...
        if (here == Terrain.straight || here == Terrain.downhill) {
          if (v.aheadSameLane != null && v.staminaFraction < 0.55) {
            return const Intent(IntentKind.draft);
          }
          return const Intent(IntentKind.surge, magnitude: 3);
        }
...
class RivalFactory {
  ...
  static List<Entrant> field({
    ...
    for (var i = 0; i < count; i++) {
      ...
      final genome = Genome.random(rng, recessiveBias: 0.08 + grade * 0.05);
