...
/// The six gene loci. Order matters: it defines the layout of [Genome.alleles]
/// and the order alleles are shown in the Hatchery, so never reorder without a
/// save migration.
...
class Allele {
  ...
  /// Command granted when expressed.
  final String traitCommand;

  /// Command granted only when homozygous.
  final String signatureCommand;

  bool get isRecessive => index == locus.alleles.length - 1;

  Color get tint => switch (locus) {
    Locus.stride => Palette.distance,
    Locus.wind => Palette.stamina,
    Locus.temper => Palette.ember,
    Locus.plumage => Palette.schoolRainbow,
    Locus.comb => Palette.effort,
    Locus.claw => Palette.momentum,
  };
...
class StatMods {
  ...
  /// Compact human-readable summary, used in the Hatchery and Codex.
  String describe() {
    final parts = <String>[];
...
class Alleles {
  ...
  /// Every allele in the game, in locus order. Used by the Codex.
