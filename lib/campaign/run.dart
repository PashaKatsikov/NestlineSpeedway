...
class Campaign {
  ...
  final int seed;
  final int grade;
  final CircuitBook map;

  /// Yard ids of the birds travelling this season.
  ...
  CircuitStop? get lastNode => path.isEmpty ? null : map.byId(path.last);

  /// Nodes the player may travel to next.
  List<CircuitStop> get available {
  ...
  void arriveAt(CircuitStop node) {
    path.add(node.id);
    cleared.add(node.id);
  ...
  /// Maneuvers added by fitted tack.
  List<String> tackManeuvers(String racerId) {
    final out = <String>[];
    for (final id in equippedOn(racerId)) {
  ...
  static Campaign fromJson(Map<String, dynamic> j) {
    final state = Campaign(
      seed: j['seed'] as int,
      grade: j['grade'] as int? ?? 0,
      map: CircuitBook.fromJson(Map<String, dynamic>.from(j['map'] as Map)),
...
class HeatPayout {
  ...
  /// Alleles the Ledger learned from this event.
